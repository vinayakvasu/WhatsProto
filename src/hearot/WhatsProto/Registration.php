...
class Registration
{
    ...
    public function __construct($number, $debug = false, $customPath = false)
    {
        ...
        $this->eventManager = new \hearot\WhatsProto\Events\Manager();
     ...
     *       a password change.
     *
     * @throws \hearot\WhatsProto\RegistrationException
     *
     * @return object
    ...
    public function checkCredentials()
    {
        if (!$phone = $this->dissectPhone()) {
            throw new \hearot\WhatsProto\RegistrationException('The provided phone number is not valid.');
        }
        $countryCode = ($phone['ISO3166'] != '') ? $phone['ISO3166'] : 'US';
        $langCode = ($phone['ISO639'] != '') ? $phone['ISO639'] : 'en';
        $host = 'https://' . \hearot\WhatsProto\Constants::WHATSAPP_EXIST_HOST;
        $query = [
      'cc'                 => $phone['cc'],
      'in'                 => $phone['phone'],
      'lg'                 => $langCode,
      ...
      'extstate' => '1',
        ...
        if ($response->status != 'ok') {
            ...
            throw new \hearot\WhatsProto\RegistrationException('There was a problem trying to request the code.');
        } else {
            $this->eventManager()->fire(
              'onCredentialsGood',
            [
    ...
    public function codeRegister($code)
    {
        ...
        // Build the url.
        $host = 'https://' . \hearot\WhatsProto\Constants::WHATSAPP_REGISTER_HOST;
        $query = [
      'cc'                 => $phone['cc'],
      'in'                 => $phone['phone'],
      'lg'                 => $langCode,
      ...
      'code'               => $code,
        ...
        if ($response->status != 'ok') {
            ...
            throw new \hearot\WhatsProto\RegistrationException('An error occurred registering the registration code from WhatsApp. Reason: ' . $response->reason);
        } else {
            $this->eventManager()->fire(
              'onCodeRegister',
            [
    ...
    public function codeRequest($method = 'sms', $carrier = 'T-Mobile5', $platform = 'Android')
    {
        ...
        // Build the token.
        $token = \hearot\WhatsProto\Utils\Functions::generateRequestToken($phone['country'], $phone['phone'], $platform);

        // Build the url.
        $host = 'https://'.\hearot\WhatsProto\Constants::WHATSAPP_CODE_HOST;
        $query = [
        'cc'                 => $phone['cc'],
        'in'                 => $phone['phone'],
        'lg'                 => $langCode,
        ...
        if ($response->status == 'ok') {
            ...
            if (isset($response->reason) && $response->reason == 'too_recent') {
                ...
                throw new \hearot\WhatsProto\RegistrationException("Code already sent. Retry after $minutes minutes.");
                ...
                $minutes = round($response->retry_after / 60);
                throw new \hearot\WhatsProto\RegistrationException("Too many guesses. Retry after $minutes minutes.");
            } else {
                $this->eventManager()->fire(
                  'onCodeRequestFailed',
                [
    ...
    protected function getResponse($host, $query)
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, \hearot\WhatsProto\Constants::WHATSAPP_USER_AGENT);
    ...
    {
        $WAData = json_decode(file_get_contents(\hearot\WhatsProto\Constants::WHATSAPP_VERSION_CHECKER), true);
        $WAver = $WAData['e'];

        if (\hearot\WhatsProto\Constants::WHATSAPP_VERSION != $WAver) {
            \hearot\WhatsProto\Utils\Functions::updateData('Constants.php', $WAver);
        }
    }
     ...
     *
     * @throws \hearot\WhatsProto\RegistrationException        Error when cannot write identity data to file.
     *
    ...
    protected function buildIdentity($identity_file = false)
    {
        ...
        if (file_put_contents($identity_file, urlencode($bytes)) === false) {
            throw new \hearot\WhatsProto\RegistrationException('Unable to write identity file to '.$identity_file);
        }
        return $bytes;
    }
